<?php
	...
	/*
	*	Сторонний скрипт отображения игровых скинов в трех режимах.
	*	mode: 1 - только голова, 2 - полностью спереди, 3 - полностью сзади.
	*/

	$skin = array(
		'dir_skins' => __DIR__.'/skins/',
		'dir_cloaks' => __DIR__.'/cloaks/',
	...
	if (!is_dir($skin['dir_skins']) or !is_dir($skin['dir_cloaks'])) exit('Путь к скинам или плащам не является папкой! Укажите правильный путь.');
	
	if (file_exists($skin['dir_skins'].$skin['user'].'.png')) $skin['skin'] = $skin['dir_skins'] . $skin['user'] . '.png';
	else $skin['skin'] = $skin['dir_skins'] . $skin['default'] . '.png';
	
	if (file_exists($skin['dir_cloaks'] . $skin['user'] . '.png')) {
		$skin['cloak'] = $skin['dir_cloaks'] . $skin['user'] . '.png';
		$skin['cloak_check'] = true;
	} else $skin['cloak_check'] = false;
	
	if (empty($skin['size'])) $skin['size'] = '128';
	
	if (empty($skin['mode'])) $skin['mode'] = '1';
	...
	if ($skin['cloak_check']) $skin['c'] = imagecreatefrompng($skin['cloak']);
	...
	if ($skin['mode'] == '1') {
		...
		if ($skin['cloak_check']) imagecopy($skin['p'], $skin['c'], 3*$skin['r'], 8*$skin['r'], 12*$skin['r'], 1*$skin['r'], 10*$skin['r'], 16*$skin['r']);
		// Face
		...
		if ($skin['cloak_check']) imagecopy($skin['p'], $skin['c'], 3*$skin['r'], 8*$skin['r'], 1*$skin['r'], 1*$skin['r'], 10*$skin['r'], 16*$skin['r']);
	...
	if ($skin['mode'] == '3') $skin['fs'] = imagecreatetruecolor($skin['size'],$skin['size']);
	else $skin['fs'] = imagecreatetruecolor($skin['size'],$skin['size']*2);
	...
	imagedestroy($skin['p']);
	imagedestroy($skin['s']);
	if ($skin['cloak_check']) imagedestroy($skin['c']);